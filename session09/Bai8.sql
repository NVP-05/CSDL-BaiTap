...
-- 3) Tạo một view tên view_highest_priced_products 
-- để hiển thị sản phẩm có giá bán (MSRP) cao nhất trong từng dòng sản phẩm (productLine) với các cột: productLine, productName, MSRP
create view view_highest_priced_products 
as
select p.productLine, p.productName, p.MSRP from products p
where p.MSRP = (select max(p2.MSRP) from products p2 where p2.productLine = p.productLine);
...
*/
select v.productLine, v.productName, v.MSRP, p.textDescription from view_highest_priced_products v
